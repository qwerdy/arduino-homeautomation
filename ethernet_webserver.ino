...
void setup () {
  ...
  low_temp = EEPROM.read(1)/TEMP_PRESCALE;
  high_temp = EEPROM.read(2)/TEMP_PRESCALE;
  doorbell_on = EEPROM.read(3);
  ovn_aktiv = EEPROM.read(0);
  if(ovn_aktiv) { TCCR1B = 0x05;                   //Start timer if activated
                  mySwitch.switchOff('d', 1, 3);   //Turn off ovn, so we know its state since default ovn_on = false
                }  	
  else          { TCCR1B = 0;    }
...
while(true) {
  ...
  else if(overflow_count == 255) { // Check temp, doing it this way so it wont delay ethernet too much!
    check_temp();
    overflow_count = 0;
...
void job_switch(char *tempBuff, int &charsReceived, boolean tcp) {
        ...
        /* Send info             $D,$D,$F,$D,$D,$F,$D,$D,$D,$D - ovn_aktiv, ovn_on, temp, low_temp, high_temp, TEMP_PRESCALE, doorbell_on, humidity, light(0-1023), sensor count */
        /* Send only humidity */ case 'h': if(!tcp) break;       DHT11.read(DHT11PIN); client.print(DHT11.humidity); client.stop(); break;
        /* Toggle timer(ovn)  */ case 'm': if(tcp)  end_TCP();   if(ovn_aktiv){ TCCR1B = 0; mySwitch.switchOff('d', 1, 3); ovn_on = false; ovn_aktiv = false; EEPROM.write(0,0); digitalWrite(LED_PIN, 0);} else { overflow_count = 0;TCCR1B = 0x05; ovn_aktiv = true; EEPROM.write(0,1);} break;
        ...
        /* Force ovn on       */ case 'i': if(tcp)  end_TCP();   force_on = 1; check_temp(); break;
        /* Force ovn off      */ case 'j': if(tcp)  end_TCP();   force_on = 2; check_temp(); break;
        ...
        /* Set high temp      */ case 'H': if(tcp)  end_TCP();   high_temp = ((tempBuff[charsReceived-3]-48)*100 + (tempBuff[charsReceived-2]-48)*10 +(tempBuff[charsReceived-1]-48))/TEMP_PRESCALE; EEPROM.write(2,high_temp*TEMP_PRESCALE); break;
        /* Set low temp       */ case 'L': if(tcp)  end_TCP();   low_temp  = ((tempBuff[charsReceived-3]-48)*100 + (tempBuff[charsReceived-2]-48)*10 +(tempBuff[charsReceived-1]-48))/TEMP_PRESCALE; EEPROM.write(1,low_temp*TEMP_PRESCALE); break;
        /* Doorbell on/off    */ case 'D': if(tcp)  end_TCP();   doorbell_on = (doorbell_on ? false : true); EEPROM.write(3,doorbell_on); break;
        /* Force temp check   */ case 'Z': if(tcp)  end_TCP();   check_temp(); break;
